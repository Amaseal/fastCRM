<script lang="ts">
	...
	import type { Client, Task } from '$lib/server/db/schema.js';
	...
	let sortColumn = $state(data.pagination.sortColumn as keyof Task | null);
	...
	$effect(() => {
		...
		sortColumn = data.pagination.sortColumn as keyof Task | null;
	...
	// Handle sorting
	function handleSort(column: keyof Task) {
		// Check if the column is sortable
		const sortableColumns: (keyof Task)[] = ['title', 'count', 'price'];
...
>
	<div class="flex w-full items-center gap-1 lg:gap-2">
		...
		<h1 class="text-base font-medium">Pabeigtie</h1>
		<Separator orientation="vertical" class="mx-2 data-[orientation=vertical]:h-4" />
		<Input
			type="text"
				...
				{#if data.tasks.length === 0}
					<Table.Row>
						<Table.Cell colspan={8} class="py-6 text-center"
							>Neviens darbs vel nav pabeigts!</Table.Cell
						>
					</Table.Row>
				{:else}
