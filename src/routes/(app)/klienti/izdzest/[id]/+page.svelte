<script lang="ts">
	...
	let { data }: { data: { item: Client } } = $props();
</script>

<svelte:head>
	<title>Izdzēst {data.item.name}</title>
</svelte:head>
		...
		<Card.Root class="custom-scroll relative max-h-[90vh] w-full max-w-md gap-2 overflow-y-auto">
			<Card.Header>
				<a
					...
					><X /></a
				>
				<h2 class="text-lg font-semibold">Vai tiešām vēlies izdzēst {data.item?.name}?</h2>
			</Card.Header>
			<Card.Content class="p-6 pb-2">
				<form method="POST" use:enhance>
					<div class="flex items-center justify-end gap-2">
						<Button href="/produkti" variant="secondary">Atcelt</Button>
